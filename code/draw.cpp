...
void Form::draw_tree(QVector<edge>res,int n,QVector<edge>g){
    scene->clear();
    ...
    for(auto ed:res){
        int u = ed.u;
        int v = ed.v;
        QPointF pt = (points[u]+points[v])/2;
        QGraphicsTextItem *t = scene->addText(QString::number(ed.dis));
        t->setPos(pt);
